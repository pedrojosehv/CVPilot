...
class ContentGenerator(LoggerMixin):
    ...
    def _generate_profile_summary(self, job_data: JobData, match_result: MatchResult) -> ReplacementBlock:
        ...
        prompt = f"""
        Create an exceptional professional summary for a CV targeting this specific role. Study the examples and create something similarly impactful.

        EXCELLENT SUMMARY EXAMPLES TO EMULATE:
        - "Data-driven product analyst adept at integrating data sources (SQL, GA4, Google Tag Manager) to optimize UX. Skilled to define and track KPIs, build insightful dashboards (Tableau, Looker Studio), and communicate actionable recommendations to stakeholders."
        - "Results-oriented professional with proven expertise in [SPECIFIC SKILLS]. Demonstrated ability to [CONCRETE ACTIONS] resulting in [MEASURABLE IMPACTS]."
        ...
        YOUR STRENGTHS:
        - Matched Skills: {', '.join(match_result.matched_skills[:10]) if match_result.matched_skills else 'strong analytical and technical capabilities'}

        JOB DESCRIPTION CONTEXT:
        {original_description[:800] + '...' if original_description else 'Focus on core responsibilities based on role type and required skills'}

        WRITING REQUIREMENTS:
        1. LENGTH: 450-550 characters (quality over strict limits - aim for 500+ chars like the examples)
        2. SPECIFICITY: Mention actual tools, skills, and methodologies from the job requirements
        3. IMPACT: Include concrete actions and measurable outcomes
        4. ROLE ALIGNMENT: Tailor content specifically to {job_data.job_title_original} responsibilities
        5. AUTHENTICITY: Write like a natural professional introduction, not a stiff corporate bio
        6. VALUE PROPOSITION: Show how your capabilities solve the employer's specific needs

        Create a summary that demonstrates deep understanding of {job_data.job_title_original} work and shows clear value through specific, measurable contributions.
        """
