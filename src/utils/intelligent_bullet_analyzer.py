...
class EnhancedBulletAnalyzer(LoggerMixin):
    ...
    def _load_enhanced_bullet_pool(self) -> Dict[str, Any]:
        ...
        try:
            ...
            profiles = {
                ...
                # Basic profile temporarily removed - using only advanced profile
                "basic": {
                    "companies": [],
                    "bullets": {}
                }
    ...
    def _extract_role_titles(self) -> Dict[str, Dict[str, List[str]]]:
        ...
        try:
            ...
            titles = {
                "advanced": {},
                # Basic profile removed - only using advanced profile
                "basic": {}
            }
    ...
    def select_appropriate_role_title(self, job_data: Dict[str, Any], profile_type: str) -> str:
        """CRITICAL FIX: Use job title directly - NO bullet pool selection"""

        # BULLET POOL RULE VIOLATION FIX:
        # This function was incorrectly selecting from bullet pool options
        # It should use the JOB TITLE directly

        job_title = job_data.get('job_title', '')

        self.log_info(f"🎯 Using job title directly: {job_title} (no bullet pool selection)")
        return job_title.strip()
    
    def _select_best_matching_title(self, job_title: str, available_titles: List[str]) -> str:
        """CRITICAL FIX: Use job title directly - NO selection from bullet pool"""

        # BULLET POOL RULE VIOLATION FIX:
        # The system should NOT select from bullet pool options
        # It should use the JOB TITLE TARGET directly

        # Return the job title as-is - this is what should be used for replacements
        return job_title.strip()
