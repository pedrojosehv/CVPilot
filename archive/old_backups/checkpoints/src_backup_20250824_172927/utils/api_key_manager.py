"""
API Key Manager for automatic rotation across all programs
...
class APIKeyManager:
    """Manages API keys with automatic rotation and rate limiting"""
    ...
    def _load_keys_from_env(self):
        ...
        if datapm_api_file.exists():
            try:
                with open(datapm_api_file, 'r', encoding='utf-8') as f:
                    ...
                    if keys:
                        self.keys = keys
                        print(f"✅ Loaded {len(keys)} API keys from DataPM API_keys.txt")
                        return
            except Exception as e:
                print(f"⚠️ Error reading DataPM API keys file: {e}")
        ...
        for env_var in env_vars:
            ...
            if keys_str:
                ...
                if keys:
                    self.keys = keys
                    print(f"✅ Loaded {len(keys)} API keys from {env_var}")
                    return
        
        # PRIORITY 3: Try local CVPilot API keys file
        ...
        if local_api_file.exists():
            try:
                with open(local_api_file, 'r', encoding='utf-8') as f:
                    ...
                    if keys:
                        self.keys = keys
                        print(f"✅ Loaded {len(keys)} API keys from local API_keys.txt")
                        return
            except Exception as e:
                print(f"⚠️ Error reading local API keys file: {e}")
        
        if not self.keys:
            print("❌ No API keys found in DataPM file, environment variables, or local files")
    ...
    def mark_error(self, key: str, error_type: str = "rate_limit"):
        ...
        if key in self.keys:
            ...
            print(f"⚠️ API key error: {error_type} for key ending in ...{key[-4:]}")
    ...
    def reset_errors(self):
        ...
        self.key_errors.clear()
        print("✅ Reset API key error counts")

# Global instance
api_key_manager = APIKeyManager()

def get_api_key(strategy: str = "round_robin") -> Optional[str]:
    """Get API key from global manager"""
    return api_key_manager.get_key(strategy)

def mark_api_error(key: str, error_type: str = "rate_limit"):
    """Mark API error for global manager"""
    api_key_manager.mark_error(key, error_type)
