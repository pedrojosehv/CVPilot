...
class ContentValidator(LoggerMixin):
    ...
    def __init__(self):
        ...
        self.forbidden_tokens = [
            ...
            "draft", "template", "placeholder",
            "lorem ipsum", "sample"  # Removed "example" and "demo" as they're too restrictive
        ]
        
        # Validation limits
        self.max_summary_length = 600
    ...
    def _validate_profile_summary(self, summary: ReplacementBlock) -> tuple[List[ValidationError], List[ValidationError]]:
        ...
        if word_count > self.max_summary_length:
            errors.append(ValidationError(
                field="ProfileSummary",
                message=f"Summary too long: {word_count} words (max {self.max_summary_length})",
                severity="error"
            ))
        elif word_count < 20:
            warnings.append(ValidationError(
                field="ProfileSummary",
                message=f"Summary too short: {word_count} words (min 20 recommended)",
                severity="warning"
            ))
    ...
    def _validate_objective_title(self, title: ReplacementBlock) -> tuple[List[ValidationError], List[ValidationError]]:
        ...
        if len(title.content) > 50:
            errors.append(ValidationError(
                ...
                message=f"Title too long: {len(title.content)} chars (max 50)",
                severity="error"
            ))
