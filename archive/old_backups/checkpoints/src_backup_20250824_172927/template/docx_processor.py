...
class DocxProcessor(LoggerMixin):
    ...
    def __init__(self):
        ...
        self.target_sections = {
            'cv_title': ['PRODUCT ANALYST', 'CURRICULUM VITAE', 'CV', 'RESUME', 'CURRÍCULUM VITAE', 'INNOVATION SPECIALIST'],
            'professional_summary': ['PRODUCT MANAGER WITH', 'PROFESSIONAL SUMMARY', 'RESUMEN PROFESIONAL', 'SUMMARY', 'EXECUTIVE SUMMARY'],
    ...
    def generate_dry_run(self, template_path: Path, replacements: Replacements, output_path: Path) -> Path:
        ...
        try:
            ...
            # Save as preview DOCX with intelligent naming
            software_list = replacements.software_list.content.split(', ') if isinstance(replacements.software_list.content, str) else replacements.software_list.content
    ...
    def _replace_cv_title(self, doc: Document, new_title: str):
        """Replace the main CV title with the job target title"""
        ...
        for paragraph in doc.paragraphs:
            ...
            if any(keyword in text.upper() for keyword in self.target_sections['cv_title']):
            ...
            if in_experience_section and text:
                ...
                if self._is_experience_job_title(text):
                    ...
                    if self._should_replace_title(text, doc, new_title):
                        ...
                        # PRESERVE THE ORIGINAL TEXT STRUCTURE - Only replace the job title part
                        # Find the job title part and replace it, keeping the rest intact

                        # Extract the original job title (first part before any dates or specializations)
                        import re

                        # Split the text by common separators (tabs, multiple spaces, dates)
                        parts = []
                        if '\t' in original_text:
                            parts = original_text.split('\t')
                        elif '   ' in original_text:  # Multiple spaces
                            parts = original_text.split('   ')
                        else:
                            ...
                            if date_match:
                                ...
                                parts = [original_text[:date_start].strip(), original_text[date_start:].strip()]
                            else:
                                # If no clear separator, treat the whole thing as the title
                                parts = [original_text]

                        # The first part should contain the job title
                        title_part = parts[0].strip()
                        remaining_parts = parts[1:] if len(parts) > 1 else []

                        # Replace the job title in the first part
                        # First, extract the base title (before any parentheses)
                        base_title_part = title_part.split('(')[0].strip()

                        # Look for common job titles to replace in the base title
                        ...
                        for old_title in ['Product Specialist', 'Product Analyst', 'Business Analyst', 'Project Manager', 'Product Owner', 'Quality Assurance Analyst', 'Quality Analyst']:
                            if old_title.lower() in base_title_part.lower():
                                job_title_to_replace = old_title
                                break

                        if job_title_to_replace:
                            # Replace the job title while preserving the rest of the text structure
                            # Replace in the original title_part (which includes parentheses)
                            new_title_part = title_part.replace(job_title_to_replace, capitalized_title)
                        else:
                            # If no specific title found, just replace the entire title part
                            new_title_part = capitalized_title

                        # Reconstruct the full text with original spacing and structure
                        if remaining_parts:
                            new_job_title = new_title_part + ('\t' if '\t' in original_text else '   ') + '   '.join(remaining_parts)
                        else:
                            new_job_title = new_title_part
                        
                        # Replace the text while preserving the original font style
                        ...
                        if original_runs:
                            ...
                            # Clear any additional runs to avoid formatting issues
    ...
    def _should_replace_title(self, text: str, doc: Document, job_target: str = None) -> bool:
        ...
        # Check if the current title is in the replaceable list
        should_replace = job_title_part in replaceable_titles
        ...
        if is_gca_context:
            # GCA Advanced Profile Rules - only replace specific titles
            should_replace = job_title_part in replaceable_titles
            self.log_info(f"   🏢 GCA context: should_replace = {should_replace}")
        elif is_loszen_context:
            # Loszen: Only replace titles that are in replaceable_titles list
            should_replace = job_title_part in replaceable_titles
            current_period = self._extract_period_from_title(text)
            self.log_info(f"   🏢 Loszen context: should_replace = {should_replace} (title: {job_title_part})")
            self.log_info(f"   📅 Current period: {current_period}")
        else:
            # NOT GCA or Loszen - DO NOT REPLACE
            should_replace = False
            self.log_info(f"   🏢 No company context: should_replace = False")
        
        # Store the selected title for later use
        if should_replace and job_target:
            # Intelligent title selection based on job target and period
            self._selected_title = self._select_intelligent_title(job_target, text, is_gca_context, is_loszen_context)
        else:
    ...
    def _select_intelligent_title(self, job_target: str, current_title: str, is_gca_context: bool, is_loszen_context: bool) -> str:
        """Intelligently select the best title based on job target and period from bullet_pool"""
        try:
            # Define period-based role options from bullet_pool.docx
            # BULLET POOL: Períodos con sus alternativas disponibles
            # Las alternativas están disponibles para cualquier empresa en ese período
            period_roles = {
            ...
            # Determine the period from the current title
            period = self._extract_period_from_title(current_title)
            
            # Get available options for this period (available for any company)
            available_options = period_roles.get(period, [])
            
            # If no alternatives available for this period, don't change
            if not available_options:
                return current_title.split('(')[0].strip()  # Return original title
            
            # Intelligent selection based on available options in bullet pool
            if available_options:
                # For titles like "digital product specialist" that can be replaced based on context
                if job_title_part in ['digital product specialist', 'product specialist']:
                    # Use the first available option from bullet pool for this period
                    return available_options[0]
                else:
                    # For standard titles, return the first available option
                    return available_options[0]
            else:
                # No alternatives available, return original title without modification
                return current_title.split('(')[0].strip()
            
        except Exception as e:
            self.log_warning(f"Error in intelligent title selection: {e}")
            return current_title.split('(')[0].strip()
    ...
    def _is_duplicate_job_title(self, text: str, new_title: str) -> bool:
        ...
        # Look for job titles that are NOT the new title but look like job titles
        # These are typically the original job titles that should be removed
        
        if not text or text.startswith('•') or text.startswith('-') or text.startswith('*'):
            return False
        
        # If this text contains the new title, it's not a duplicate
        if new_title in text:
            return False
        
        # Look for job title patterns that indicate this is an original job title
        job_title_patterns = [
            'Product Analyst', 'Product Operations Specialist', 'Quality Assurance Analyst',
            'Product Manager', 'Project Manager', 'Business Analyst', 'Data Analyst',
            'Senior', 'Junior', 'Lead', 'Principal', 'Head', 'Director'
        ...
        # Check if it contains job title keywords but is NOT the new title
        has_job_keywords = any(pattern in text for pattern in job_title_patterns)
        
        # Check if it looks like a job title format (short, no bullet points, etc.)
        looks_like_title = (
            len(text) < 100 and
            not text.endswith('.') and
            not any(word in text.lower() for word in ['experience', 'responsibilities', 'duties', 'managed']) and
            any(sep in text for sep in ['(', ')', '–', '—', '\t'])
        )
        
        return has_job_keywords and looks_like_title
